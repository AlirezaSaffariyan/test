...
dependencies = {}
zoe = torch.hub.load(
    "/home/ars/projs/side-projs/ZoeDepth/ZoeDepth/",
    "ZoeD_N",
    source="local",
    pretrained=True,
)
...
def process(file_name):
    ...
    for i in range(len(filters)):
        ...
        for x in x_coords_all:
            for y in y_coords_all:
                ...
                depth = dependencies["zoe"].infer_pil(
        ...
        tiled_depth_map = Image.fromarray(
            (2**16 * 0.999 * compiled_tiles / np.max(compiled_tiles)).astype("uint16")
        )
    ...
    combined_image.save(file_name.split(".")[0] + "_depth.png")
